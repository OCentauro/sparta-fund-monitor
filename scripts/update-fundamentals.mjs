...
// 3. Função para extrair a Cota Patrimonial
async function extractCotaPatrimonial(url, ticker) {
  try {
    ...
    // Fallback específico e MAIS RIGOROSO para MXRF11
    if (ticker === 'MXRF11') {
      // Tenta procurar por "valor patrimonial" seguido de números, mas loga o que encontrou para depuração
      const regexXp = /valor\s+patrimonial[\s\S]{0,150}r\$\s*([0-9]{1,2}[.,][0-9]{2})/gi;
      const matchesXp = [...html.matchAll(regexXp)];
      
      if (matchesXp.length > 0) {
        // Pega a ÚLTIMA ocorrência encontrada na página (geralmente a mais recente/atual)
        const lastMatch = matchesXp[matchesXp.length - 1];
        console.log(`🔍 [DEBUG MXRF11] Trecho encontrado no HTML: "${lastMatch[0]}"`);
